...
test_data = test_df_close.values


# Prepare the test set similar to training set
x_test = []

for d in range(N_PRED_DAYS, len(scaled_test_data)):
    x_test.append(scaled_test_data[d - N_PRED_DAYS: d, 0])
...
plt.figure(figsize=(14, 7))
plt.plot(test_df.index[N_PRED_DAYS:], y_test, color='blue', label='Actual Price')
plt.plot(test_df.index[N_PRED_DAYS:], predictions, color='red', label='Predicted Price')
